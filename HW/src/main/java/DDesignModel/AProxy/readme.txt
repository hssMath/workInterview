...
1、代理类和原类做的事情是一样的。
2、原类不能被修改，通过代理类在原类的功能基础上，进行一个功能增强。
总结：在某些情况下，目标类不能被客户类直接访问，在目标类和客户类之间创建一个代理类，客户类通过代理类来间接访问目标类。
二、举例
eg1、发短信功能：三大通信运营商具有发短信的功能，项目本身不具备发短信的能力，但是，可以通过运行商提供的接口，发送定制化的内容短信给客户。
    发短信的功能接口类似一个黑盒，我们可以在发送前后写入自己的业务逻辑，实现功能的增强。
三、代理出现的原因：
    1、功能增强
    2、控制访问
