...
/**
 * 将含有m个非0元素+n个0的递增数组M和n个元素的递增数组N进行合并，然后输出给新数组M(m+n)，保证新数组M(m+n)是递增的。
 * 说明：
 *      1.合并两个数组之后的新数组长度是确定的，就可以新建一个长度为m+n的临时数组。
 *      2.利用2个数组已经是递增的特点进行排序。
 *      3.M数组的非0元素m与N数组的n个元素不一定相等。
 * 解法（双指针）：一个for循环，给2个数组分别设置一个下标，取两个数组的第一个元素开始比较，小的数给for循环的第i个元素，然后把小数的数组下标+1，
